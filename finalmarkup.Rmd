...
# The dataset
The raw data were sourced from \textit{Morningstar} capturing the Median return and the Average returns with a daily frequency.

The difference between the median returns and the average ones are substantial by construction. While the average returns are built by Morningstar on an unbiased basis (in terms of survivorship bias) daily, the median ones are calculated manually by the \textit{OMGI Investment Risk and Performance Team} filtering each sector as of today and going back in time calculating each period median returns: the result is a time series of survivorship biased returns.

The filter admits only the {xxxxxx} class for each product using (as an example for the \textit{IA Mix 0-35} sector) the following definition:
```
"SUPER_CLIENT:/INDEX_LISTS/PEER_GROUPS/Sector - IMA - Mixed Investment 0-35 Shares - PTX"
```
Specturm funds prices are taken as delivered by Citi. The Specturm prices are published by class on a Bid/Mid/Offer basis. For this exercise the Mid price was used for the OA class.
...
Four datasets are considered (from top to bottom, left to right) where different start/end dates has been used:
\begin{enumerate}
...
The same logic is applied using sectors averages (Panel 2). In this case we believe the reference should makes the comparison less aggressive. 
By using the sector average returns we compare the funds with the sectors statistics wherer the funds that were closed out (or moved to a different sector) are included; in all likelihood you would expect those returns to be lower than the one obtained with the survivorship-biased medians.

...
```{r VolatilityCheck, echo=FALSE, warning=FALSE}
...
```{r EquityFundsCheck, echo=FALSE, warning=FALSE}
...
```{r plainLm, echo=FALSE, warning=FALSE, results='markup'}
...
```{r plainLmChart, echo=FALSE, warning=FALSE}
...
```{r VIXSetsplit, echo=FALSE, warning=FALSE}
ggplot(RegSet, aes(x=Exposure, y = SdRet 
...
SDonEXP <- lm(SdRet ~ Exposure + VIX, data = RegSet)
summary(SDonEXP)
```
